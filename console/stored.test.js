...
import { reads as adminReads, mount } from './test/mockAdmin';
...
test('selected-key reads and clear use auth, one DELETE, and authoritative refresh even on ambiguous failure',async()=>{
 let value=structuredClone(stored), writes=0;
 const request=vi.fn(async(url,init)=>{
  ...
  return adminReads(fixture)(url);
 });
 app=mount(fixture,{token:'admin-test',request});await vi.advanceTimersByTimeAsync(0);expect(request).toHaveBeenCalledTimes(6);
...
for(const status of [409,429,503])test('clear HTTP '+status+' is not described as an ambiguous network outcome',async()=>{
 ...
 app=mount(fixture,{request});await vi.advanceTimersByTimeAsync(0);
 document.querySelector(`[data-key="${fixture.keys[0].id}"]`).click();await vi.advanceTimersByTimeAsync(0);
...
test('stored read waits for dashboard reads and drawer polling uses four seconds',async()=>{
 ...
 const request=async(url)=>{active++;maximum=Math.max(maximum,active);await new Promise(r=>setTimeout(r,20));active--;
  ...
  return adminReads(fixture)(url);};
 app=mount(fixture,{request});await vi.advanceTimersByTimeAsync(30);
...
test('polling cannot silently replace the counts and cursor the host confirmed',async()=>{
 let value=structuredClone(stored),sent;
...
test('committed clear warning and actual skipped count survive a failed refresh',async()=>{
 ...
 app=mount(fixture,{request});await vi.advanceTimersByTimeAsync(0);document.querySelector(`[data-key="${fixture.keys[0].id}"]`).click();await vi.advanceTimersByTimeAsync(0);
 document.querySelector('[data-action="stored-confirm"]').click();document.querySelector('[data-action="stored-clear"]').click();await vi.advanceTimersByTimeAsync(0);
...
test('invite drawer keeps two-second polling and does not fetch Stored',async()=>{
 const request=vi.fn(async(url)=>{expect(url).not.toContain('/stored');return adminReads(fixture)(url);});
 app=mount(fixture,{request});await vi.advanceTimersByTimeAsync(0);document.querySelector('[data-action="new"]').click();const before=request.mock.calls.length;await vi.advanceTimersByTimeAsync(8100);expect(request.mock.calls.length-before).toBe(24);
});
...
for(const outcome of ['refusal','timeout','unauthorized'])test('Stored GET handles '+outcome+' without raw JS errors',async()=>{
 ...
 app=mount(fixture,{request});await vi.advanceTimersByTimeAsync(0);document.querySelector(`[data-key="${fixture.keys[0].id}"]`).click();await vi.advanceTimersByTimeAsync(outcome==='timeout'?5100:0);
