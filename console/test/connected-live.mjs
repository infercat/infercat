// 094 opt-in: two friend devices and the local/remote console on one fresh own host.
...
const adapter=process.env.INFERCAT_PROOF_TUNNEL;
if(!binary||!web||!adapter)throw new Error('set INFERCAT_PROOF_BINARY and INFERCAT_PROOF_WEB');
...
try{
 ...
 for(let device=0;device<2;device++){
  ...
  for(let i=0;i<150&&!output.includes('http://');i++)await sleep(200);
  const origin=output.trim().split('\n')[0];assert.ok(origin.startsWith('http://127.0.0.1:'));assert.equal((await fetch(origin+'/me',{headers:{Authorization:'Bearer '+secret}})).status,200);
 }
 ...
 await local.goto(url);await local.locator('#friends').waitFor();await local.locator(`[data-key="${friend.key_id}"] .c-now`).filter({hasText:'connected · idle · 2 devices'}).waitFor();
 ...
 assert.ok((await remote.locator(`[data-key="${friend.key_id}"] .c-now`).innerText()).includes('connected · idle · 2 devices'));report.push('Two real friend peers: local and remote table both show connected · idle · 2 devices.');
 ...
 await remote.locator(`[data-key="${friend.key_id}"]`).click();assert.ok((await remote.locator('.drawer .nowline').innerText()).includes('connected · idle · 2 devices'));await remote.locator('#limit-rpm').fill('9');await remote.locator('form[data-form="limits"] button[type="submit"]').click();await local.locator(`[data-key="${friend.key_id}"]`).filter({hasText:'9 rpm'}).waitFor();report.push('Browser via real WASM/relay: PATCH /console/api/keys/'+friend.key_id+' -> local row 9 rpm');await remote.locator('button[data-close]').click();
 ...
 capture.status.tunnel.sessions=[]; // Peer identities are not part of the console fixture.
 if(process.env.CAPTURE_CONSOLE_FIXTURE==='1')await writeFile(new URL('../../web/src/fixtures/console/current-085-dev.json',import.meta.url),JSON.stringify(capture,null,2)+'\n');
