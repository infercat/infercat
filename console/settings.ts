...
export function settingsSection(data:Snapshot,lang:Lang,ui:SettingsUI,pending:boolean):string {
 ...
 ${field('console','console_address','console_hint')}<div class="form-actions"><button type="submit" class="secondary" ${pending||!count?'disabled':''}>${t('save')}</button><span class="saved" role="status">${escape(ui.error||'')||t(pending?'working':ui.saved?'settings_saved':count===1?'one_change':'changes',count)}</span></div>
 ...
 ${ui.confirm?`<div class="confirm remote-confirm"><p>${t('off_confirm')}</p><div class="btns"><button type="button" class="primary small" data-action="remote-off" ${pending?'disabled':''}>${t('turn_off')}</button><button type="button" class="ghost tiny" data-action="remote-keep" ${pending?'disabled':''}>${t('keep_on')}</button></div></div>`:''}
