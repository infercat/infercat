...
export function limitsForm(data: Snapshot, lang: Lang, ui?: DrawerState, key?: Key, pending = false): string {
 ...
 <p class="field-hint">${t('limit_hint')}</p><div class="dactions"><button type="submit" class="${ui?.mode === 'mint' ? 'primary' : 'secondary'}" ${disabled ? 'disabled' : ''}>${t(ui?.mode === 'mint' ? 'mint_submit' : 'save_limits')}</button>${key && key.status !== 'revoked' ? `<span class="note">${t('in_force',key.name)}</span>` : ''}</div>${message(lang,ui,pending)}</div></form>`;
}
...
export function specialDrawer(data: Snapshot, lang: Lang, ui: DrawerState, pending: boolean): string {
 ...
 const title = isAdmin ? t(ui.rotated?'admin_new':'admin_title') : once ? t(ui.rotated ? 'rotate_title' : 'once_title',once.name) : t('mint');
 ...
 <p class="meta">${isAdmin?t('admin_meta'):escape(once.key_id)} · ${returnedKey ? `<b>${t(`s_${returnedKey.status}`)}</b> · ` : ''} ${t(ui.rotated ? 'rotated_line' : 'minted_now')}</p>${isAdmin?`<p class="trust">${t('admin_trust',data.settings.name)}</p>`:''}<div class="once"><span class="tag">${t('once_tag')}</span><p>${t('once_p')}</p><p class="code">${once.invite.split('.').map((part,i) => `<span class="${i === 2 ? 'sec-part' : ''}">${escape(part)}</span>`).join('<span class="dot">.</span>')}</p><div class="once-grid"><div><div class="btns">${once.link ? button(lang,'copy-link',ui.copied === 'link' ? 'copied' : 'copy_link',false,'primary small') : ''}${button(lang,'copy-code',ui.copied === 'code' ? 'copied' : isAdmin ? 'admin_copy' : 'copy_code',false)}</div>${once.link ? `<p>${isAdmin?t('admin_send_live'):t('send_p',once.name)}</p><p class="link">${escape(once.link.length > 60 ? once.link.slice(0,40) + '…' + once.link.slice(-8) : once.link)}</p>` : `<p>${t('no_web',once.name)}</p>`}</div><div>${ui.qr || ''}<p class="qrcap">${t(ui.qrFailed ? 'qr_failed' : 'scan')}</p></div></div></div>${message(lang,ui,pending)}${isAdmin?`<div class="dsec"><span class="field-label">${t('admin_opens')}</span><p>${t('admin_scope')}</p></div>`:''}${l ? `<div class="dsec"><span class="field-label">${t('d_limits')}</span><p class="nowline">${line}</p><p class="field-hint">${t('minted_limits',once.name)}</p></div>` : ''}<div class="dactions">${button(lang,'done','done',false,'primary')}<span class="note">${t(isAdmin?'admin_lost':'lost_h')}</span></div>
 ` : isAdmin ? message(lang,ui,pending) : `<label class="field"><span class="field-label">${t('new_name')}</span><input id="invite-name" name="name" form="key-limits" required value="${escape(ui.name)}" ${pending ? 'disabled' : ''}><span class="field-hint">${t('name_hint')}</span></label>${limitsForm(data,lang,ui,undefined,pending)}${ui.duplicateId ? `<button type="button" class="ghost" data-key="${escape(ui.duplicateId)}">${t('rotate')}</button>` : ''}`}</aside>`;
}
