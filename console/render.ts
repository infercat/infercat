...
export function render(data: Snapshot | null, lang: Lang, selected: string | null, stale: number | null, now = Date.now(), hasToken = true, refreshedSeconds = 2, ui?: DrawerState, pending = false, settingsUI:SettingsUI = {draft:{}}, remote?:RemoteState): string {
 ...
 if (k) {
  ...
  <div class="dsec"><span class="field-label">${label('d_usage')}</span>${usageTable(a,b,true,k.last_seen)}</div>${storedSection(ui?.stored,lang,pending,now)}${keyActions(lang, ui, k, pending)}<p class="dfoot">${t(k.status === 'revoked' ? 'revoked_help' : 'hash_only')}</p>${k.status === 'revoked' ? `<button class="primary" data-action="new">${t('mint')}</button>` : ''}</aside>`;
 }
 ...
 ${current.length ? keyTable(current) : `<p class="empty">${t('no_keys')}</p>`}${revoked.length ? `<details class="dis"><summary>${t(revoked.length === 1 ? 'revoked_one' : 'revoked_n',revoked.length)}</summary>${keyTable(revoked,true)}</details>` : ''}<p class="foot-line">${t('counts_line')}</p></div></section>
