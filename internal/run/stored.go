...
func (s *Store) stored(key string) (StoredData, error) {
	...
	out := StoredData{KeyID: key, Cursor: fmt.Sprintf("%s:%d", v.Epoch, v.Seq), Kinds: map[string]StoredKind{}, Total: len(v.Runs), Bytes: v.encodedBytes, Budget: s.budget(), Reserved: s.reservedBytes(key), ImageBudget: ImageBudget, Runs: []StoredRun{}}
