...
// Rejected requests are not logged, so a friend retrying against a 429 does not dig the hole deeper.
...
// admit atomically checks per-key concurrency, RPM, TPM (with the pre-check prompt tokens), and the
// daily budget, and on success counts the admission and takes one in-flight slot. The caller must
// call release exactly once afterwards. promptTokens is the pre-check count so a request that alone
// would exceed the remaining minute or day is refused with the numbers in the message.
func (l *limiter) admit(id string, lim keys.Limits, promptTokens int) *gwError {
	st := l.state(id)
	st.mu.Lock()
	defer st.mu.Unlock()
	now := l.now()
