...
| `runs/<key-id>/images/<run-id>` | 生成的 PNG/JPEG：每张最多 8 MiB，最多保留 7 天，或直到主机所有者清除此密钥保存的数据，每把密钥另有 256 MiB 图片空间，满时先淘汰最旧输出。参阅[图片主机配方](IMAGES.zh-CN.md)。 |
...
图片文件权限为 0600，目录为 0700。任务快照 `runs/<key-id>/state.json` 只保存图片的元数据；图片空间独立于每把密钥的 64 MiB 状态上限。Discard 立即删除输出；过期、淘汰或删除后的标记保留到任务记录过期。启动清理会处理有删除依据的路径；快照仍有记录时会清理未被记录命名的图片文件，空快照中的未知文件仅报告供人工检查。中断的任务不会自动重放。

图片空间上限计算仍可读取的输出。无法删除的旧文件会记录错误并在下次清理时重试，暂时可能占用上限之外的磁盘空间；`infercat status` 分别显示等待重试清理的路径数和待人工检查的条目数。状态快照目录和图片目录中的 `.run-*` 临时文件会在加载或清理时回收。
