...
默认每把密钥最多 **8 张图片排队**、每个 UTC 日 **20 张图片**；缺失或为零的字段采用这两个默认值；每日限额为负表示不限（`-1`）；排队限额为负时采用共享的未结束任务上限，`/me` 返回 16。示例将 Alice 改为排队 4 张、每天 10 张。`keys list` 显示 IMAGES/DAY 和实际 IMAGE QUEUE 上限（负值或大于 16 时显示 16）。朋友的图片列表、任务轮询和已接纳的 HTTP 提交消耗 RPM（每个批次或同步调用计一次，拒绝会退还）；缩略图和输出 GET 不消耗 RPM；后台图片生成只使用执行容量和每日额度预留；不占用密钥的文字/语音并发计数，也不消耗聊天 token。单次交互请求排在批量种植任务前面；已经开始生成的图片不会被抢占。

每天的额度在批次接收时整批预留：要么所有提示词排队，要么以 429 整批拒绝，不创建任何任务行。尚未结算的预留跨 UTC 午夜继续有效，已发出的任务计入结算当天。重启会中断未结束任务，不会重放排队工作。

取消排队任务不扣图片额度。生成中取消会等这张图完成，保留图片并扣一张。引擎明确失败且没有输出时释放预留；已发出但结果不确定时扣一张。响应丢失不会触发自动重发。主机一次只发送一个图片生成请求，等待完整 HTTP 响应的时间最多为 15 分钟。已发送但丢失结果的请求返回 `image_abandoned` 并计数；发出下一个请求前，主机最多等待 60 秒，直到一次新的健康探测成功。未恢复时，这个尚未发出的任务以 `upstream_down` 失败并释放预留；后续任务在引擎恢复前会立即拒绝。恢复窗口每 3 秒自行执行一次有时限的探测。宽限期取 3 分钟和本次主机进程内成功存储的生成任务最长实测耗时的两倍中的较大值，最多 10 分钟，低于请求的 15 分钟上限；主机启动时从 3 分钟开始，后续较快的成功不会缩短宽限期。在宽限期内，重叠的探测失败保留上次健康状态；超过宽限期后，探测失败会标记离线、撤下图片能力，并以 503 和 `Retry-After` 拒绝新批次。若健康状态在接纳与发出之间转为离线，任务保留原预留并等待这一次有时限的恢复，不会重新预留额度。主机在发出请求后停止，记录为已计数的 `image_abandoned`；发出前停止则记录为已释放预留的 `interrupted`。远程服务可能在断线后继续计算，健康探测不能证明它已停止。

出现两次本应扣额度的生成失败、期间没有有效的成功生成时，引擎进入 **suspect（可疑）** 状态。前两次扣额度，此后在可疑状态下这类失败不扣额度；包括非空但无效的图片输出，不限于连接丢失。计入保护的失败依次推进 1、2、4、8 分钟的退避，最多 15 分钟；明确无输出的结果释放额度，不推进退避；达到 12 MiB 的响应例外，释放额度但记录日志并计入可疑状态；仅探测成功不会清除状态，成功生成才会清除。日志记录状态变化，`infercat status` 显示失败次数（包括首次失败）和重试等待是否已结束。图片引擎仍提供能力且处于可疑状态时，`/me.host.images.retry_at` 返回可选的未来时间戳，已经过期的时刻不再返回。该保护保存在进程内，主机重启时重置，已保留的用量不重置。生成后的主机存储故障记录为 `storage_failed`（同步接口为 HTTP 500） 并释放额度，不训练宽限期，也不改变引擎保护状态；成功必须包含有效图片已存储。任务存储因故障停止接纳某把密钥时，也返回 `storage_failed`；请重试，若持续失败，主机的磁盘需要检查。失败任务保留网关自己编写的错误说明和重试秒数，不保留引擎原始错误片段。每把密钥的排队和每日额度先检查，通过后才遍历主机任务；退还 RPM 的拒绝不会强迫执行该遍历。

输出仅为 PNG/JPEG，**每张最多 8 MiB**，私密保存在[数据目录](DATA-DIRECTORY.zh-CN.md)下的 `runs/<key-id>/images/<run-id>`。保留**最多 7 天，或直到主机所有者清除此密钥保存的数据**，同时受独立的**每把密钥 256 MiB 可读取图片空间**限制，满时先淘汰最旧的输出。操作系统拒绝删除的旧文件可能暂时超出可读取图片的空间上限；主机会记录错误、在下次清理时重试，并在 status 中显示待清理文件数。Save 下载副本，Discard 删除主机上的输出。图片已丢失的标记保留到任务记录过期。主机所有者清理时也会删除任务记录：任务将无法找到，不会留下图片已丢失的标记。原有任务上限仍适用：每把密钥 16 个未结束任务、每台主机 64 个，每把密钥保留 100 条记录。
...
在提供图片工具的主机上，可以直接在对话中请求生成图片。主机同时报告图片模型和 `host_tools: ["make_image"]` 时，应用会自动启用，无需对话开关。回复照常流式显示，工具步骤和每张图片的任务行在回复下方显示。图片也会出现在“图片”中，并链接回提问的回合。停止回复不会取消已经提交的图片；请使用该图片任务行的取消按钮。
...
- **引擎不应答或看不到图片按钮：** 检查 `/v1/models`、引擎终端、基础 URL 和两层模型白名单。修改已保存参数后重启 `serve`，定期探测会发现稍后启动的引擎。健康且已共享的图片模型可用之前，`/me.host.images` 不存在。仅支持文字的旧主机或应用不会提供该按钮。列表、单个任务读取和取消、图片删除消耗 RPM；合并刷新请求，并在 429 时遵守 `Retry-After`。输出 GET 不消耗 RPM，也不占聊天并发槽；它使用独立的每密钥读取槽，保持到传输结束，最多 4 个并发，超出返回 429 和 `Retry-After: 1`。不存在的任务读取和取消会退还 RPM，缩略图不会耗尽聊天 RPM。
- **只返回 URL 被拒绝：** 服务必须返回 `data[0].b64_json`；Infercat 不会抓取远程图片 URL。使用上文固定版本的原生接口。
- **输出过大或损坏：** 解码后的 PNG/JPEG 每张不得超过 8 MiB、每边不超过 4096 像素；接口请求的是 1024×1024。没有非空 `b64_json` 候选项的结果（包括仅 URL）会被拒绝且不扣额度。非空候选项的 base64 损坏，或图片字节无效、超限，会计入可疑保护；除非该保护免除扣款，否则扣一张每日额度。解析前达到 12 MiB 的响应会被拒绝并释放额度，但会记录日志并计入引擎可疑保护。
