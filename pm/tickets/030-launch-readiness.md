...
- **2026-09-03 03:44 EDT** — README, CHANGELOG, SECURITY/CONTRIBUTING/CODE_OF_CONDUCT, three issue
  forms + PR template, `ci.yml` and `release.yml`, `docs/RELEASE.md`, `docs/RENAME.md`, ticket 034
  written. `gh repo edit`: description + 13 topics set (homepage left for F3). The cask generated by
  `make release-dry` installed from a throwaway local tap with `file://` URLs, ran `version`, carried
  no quarantine attribute, uninstalled, untapped. The browser on this laptop is not signed in to
  GitHub (README URL → 404 under "Sign in"), so the README evidence is GitHub's Markdown API output
  rendered locally, and the social preview upload is left as the documented manual step.
- **2026-09-03 03:52 EDT** — Fresh clone (`tmp/bn030-clone`, 3733111): everything green except
  `pnpm lint` — the two new dev scripts used `getComputedStyle`/`NodeFilter` bare inside
  `page.evaluate` against the hand-listed eslint globals; CI run 33729356606 failed on the same step
  (Go step passed on ubuntu). Qualified with `window.`, lint clean; rebased onto `ebc1146` (033, a
  ticket file); pushed with `--force-with-lease` (the lane was rebased after its first push); CI run
  33729877717 on `620cee0` is the one that counts. My host on :6830 and the static server on :6831
  stopped; the founder's host and llama-server untouched throughout.
- **2026-09-03 03:58 EDT** — CI run 33729877717: lint green, then `notices-check` red on ubuntu while
  green here. Cause: `go-licenses report` walks the tree **for the platform it runs on** — Linux adds
  dbus/netlink/rtnetlink/ethtool/socket and drops the macOS-only certstore — so a file generated on a
  laptop could never verify in CI (017's latent defect; law 4 exposed it). Class fix in
  `hack/notices.sh` (outside my scope list, declared): the report is the union over the five release
  targets in `.goreleaser.yaml`. `make notices` → 49 Go modules (was 40): the nine new rows are the
  Linux ones plus Windows' wingoes, go-winio, wintun, winipcfg — all permissive, all missing from the
  Linux/Windows archives until now. `make notices-check` OK; pushed for run 3.
...
- **Outside the scope list, necessary for promise 3:** `hack/notices.sh` (017's) reported the Go
  tree for the platform it ran on, so the committed notices could never verify on a Linux CI runner
  and — the real problem — the Linux and Windows archives were shipping without nine of their
  dependencies' notices. It now reports the union over the five release targets; the file grew from
  40 to 49 Go modules, all permissive. Alternative rejected: macOS runners (10× the minutes, and the
  defect stays).
...
- The notices still cover two of the three things that ship: the wasm bridge inside
  `web-<version>.zip` has its own Go tree (`GOOS=js GOARCH=wasm`, tailcat's omit tags) that
  `hack/notices.sh` does not report. Same loop, one more target with `GOFLAGS=-tags=…`; 017's
  successor.
...
## Ruling (PM, 2026-09-03 05:00)

**Landed** on main (merge of `b3b64e2`; two textual conflicts with 026/033 resolved by the PM keeping
both sides; go/web/notices/launch-check green, printed). Accepted as declared: the notices union over
five release targets (a real launch defect caught by CI: Linux/Windows archives lacked nine notices);
`homebrew_casks` instead of the deprecated `brews`; social preview left as a manual step (no API);
nothing signed (only an Apple Development identity exists) → 034 notarization drafted. Note for the
founder: CI runs on every branch push (~11 min); acceptable until the repo is public, then restrict to
main + PRs.
