...
describe('invite', () => {
  ...
  it('is case-sensitive in the parts it keeps', () => {
    expect(decodeInvite(`ic1.${ADDR}.${SECRET.toUpperCase()}`).secret).toBe(SECRET.toUpperCase());
  });

  const table: [string, string, InviteErrorCode][] = [
    ['empty', '', 'empty'], // Go says ErrPrefix here; see the note in invite.ts
    ['whitespace_only', ' \n\t', 'empty'], // ditto
    ['no_dots', 'ic1', 'wrong_part_count'],
    ['missing_prefix', `${ADDR}.secret`, 'missing_prefix'],
    ['wrong_prefix', `xx1.${OK}`, 'missing_prefix'],
    ['prefix_case', `IC1.${OK}`, 'missing_prefix'],
    ['prefix_ic_only', `ic.${OK}`, 'missing_prefix'],
    ['prefix_icx', `icx.${OK}`, 'missing_prefix'],
    ...
    ['trailing_dot', `ic1.${OK}.`, 'wrong_part_count'],
    ['leading_dot', `.ic1.${OK}`, 'missing_prefix'],
    ['empty_addr', 'ic1..secret', 'empty_part'],
    ['empty_secret', `ic1.${ADDR}.`, 'empty_part'],
    ['both_empty', 'ic1..', 'empty_part'],
    ['addr_no_tc', 'ic1.ABC.secret', 'bad_addr'],
    ['addr_tc_only', 'ic1.tc.secret', 'bad_addr'],
    ['addr_bad_char', 'ic1.tcAB+C.secret', 'bad_addr'],
    ['addr_space', 'ic1.tcAB C.secret', 'bad_addr'],
    ['addr_nul', 'ic1.tcAB\x00C.secret', 'bad_addr'],
    ['addr_unicode', 'ic1.tcABÇ.secret', 'bad_addr'],
    ['secret_plus', `ic1.${ADDR}.sec+ret`, 'bad_secret'],
    ['secret_slash', `ic1.${ADDR}.sec/ret`, 'bad_secret'],
    ['secret_padding', `ic1.${ADDR}.secret=`, 'bad_secret'],
    ['secret_inner_space', `ic1.${ADDR}.sec ret`, 'bad_secret'],
    ['secret_zero_width', `ic1.${ADDR}.sec\u200Bret`, 'bad_secret'],
...
// Promise 14: the host's CLI prints `<app>/#ic1.…`; the page fills the field from it.
...
describe('maskInvite', () => {
  const invite = `ic1.tco2FwWCCefFIUTGpLZzxLjFkzfi5BnYk9Lr2zui9eL.${'D'.repeat(38)}N96as`;

  it('shows enough to recognise and not enough to use', () => {
    expect(maskInvite(invite)).toBe('ic1.tco2…N96as');
